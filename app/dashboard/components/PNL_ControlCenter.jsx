...
const tk = (d) => ({
  ...
  // tooltip-specific shadow — heavier
...
function ColFilterDropdown({ options, selected, onToggle, onClear, t }) {
...
// ─── Status Cell — tooltip via React Portal ───────────────────────────────────
// Uses createPortal to render the tooltip at document.body level so it
// is never clipped by overflow:hidden on table containers.
function StatusCell({ statusInfo, t, d, onClick }) {
  ...
  // Compute tooltip position: prefer above, fall back to below
  const showTooltip = (e) => {
    if (status === "EMPTY") return;
    const rect = btnRef.current?.getBoundingClientRect();
    if (!rect) return;
    // anchor to button center-top
    setTip({ x: rect.left + rect.width / 2, y: rect.top });
  };
  const hideTooltip = () => setTip(null);

  const TOOLTIP_W = 220;
  const TOOLTIP_APPROX_H = hasNotes ? 130 : 80;

  // Portal tooltip — always on top of everything
  const tooltipEl = tip && typeof document !== "undefined" && createPortal(
    (() => {
      // Position: above button by default
      const spaceAbove = tip.y;
      const above = spaceAbove > TOOLTIP_APPROX_H + 12;
      const top = above
        ? tip.y - TOOLTIP_APPROX_H - 10
        : tip.y + 40 + 6; // below button

      // Clamp horizontally within viewport
      let left = tip.x - TOOLTIP_W / 2;
      left = Math.max(8, Math.min(left, window.innerWidth - TOOLTIP_W - 8));

      const arrowLeft = tip.x - left - 6;
        ...
        >
          ...
          {above ? (
            ...
            <div style={{
              position: "absolute",
              top: "100%",
              left: Math.max(6, Math.min(arrowLeft, TOOLTIP_W - 18)),
              width: 0, height: 0,
            ...
            <div style={{
              ...
              borderLeft: "6px solid transparent",
              borderRight: "6px solid transparent",
              borderBottom: `6px solid ${d ? "#1A1A1D" : "#FFFFFF"}`,
            }} />
          )}
          ...
          {hasNotes && (
            <div style={{
              fontSize: 11.5, color: d ? "#D0D2D8" : "#374151", lineHeight: 1.5,
              ...
              <span style={{
                ...
                textTransform: "uppercase",
                color: t.mid, display: "block", marginBottom: 4,
              }}>
                Keterangan
              </span>
              {notes}
...
export default function PNLControlCenter({ theme, masterData, activeYear, activeMonth, onOpenBranch }) {
  ...
  const clearAll = () => {
    setSearch(""); setFType([]); setFRegion([]);
    setFPartner([]); setFBranch([]); setFDoneCount([]);
  };

  const hasFilter = Boolean(
    search || fType.length || fRegion.length ||
    fPartner.length || fBranch.length || fDoneCount.length
  );
  ...
  const rows = useMemo(() => {
    ...
    if (fRegion.length > 0)   list = list.filter(r => fRegion.includes(r.region));
    if (fDoneCount.length > 0) {
      list = list.filter(r => {
        const fc = r.statuses.filter(s => s === "FINALIZED").length;
        return fDoneCount.includes(fc);
      });
    }
    list = [...list].sort((a, b) => {
  ...
  // TH with ExcelFilter, overflow: visible so dropdowns aren't clipped
  const TH = ({ colIdx, sortKey, children, filterConfig }) => {
    const isActiveSort = sortCol === sortKey;
    return (
      <th style={{
        position: "sticky", top: 0, zIndex: 20,
        width: colWidths[colIdx], minWidth: colWidths[colIdx],
        padding: "0 14px", height: 42, textAlign: "left",
        ...
        <div style={{
          ...
          {filterConfig && (
            <ExcelFilter {...filterConfig} t={t} d={d} />
          )}
        </div>
      </th>
    );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {[
          { label: "Pencapaian",   value: `${pct}%`, sub: `${finalCells} dari ${totalCells} laporan selesai`,  color: t.green, bg: t.greenBg },
          ...
          { label: "Belum Diisi",  value: emptyCells,  sub: "laporan kosong",                                   color: t.mid,   bg: "transparent" },
        ].map(c => (
          <div key={c.label} style={{
            padding: "18px 20px", borderRadius: 12,
            border: `1px solid ${t.line}`, background: t.card, boxShadow: t.shadow,
          }}>
      ...
      <div style={{ borderRadius: 12, border: `1px solid ${t.line}`, background: t.card, padding: "16px 20px", boxShadow: t.shadow }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: t.mid }}>Progress s.d. {activeMonth}</span>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}>
          ...
          <span style={{ fontSize: 11, color: t.lo }}>{visibleMonthCount} bulan ditampilkan</span>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          padding: "10px 18px", borderBottom: `1px solid ${t.line}`, background: t.thead,
        }}>
          {[
        ...
        {loading ? (
          ...
          // overflow: visible on the scroll container does NOT prevent scrolling;
          // we keep overflowX auto but allow the tooltip to escape via portal.
          <div style={{ overflowX: "auto", overflowY: "auto", maxHeight: "60vh" }}>
            <table style={{ borderCollapse: "collapse", fontSize: 13, tableLayout: "fixed", width: "max-content", minWidth: "100%" }}>
              <colgroup>
                {colWidths.slice(0, 5 + visibleMonthCount).map((w, i) => (
              ...
              <thead>
                <tr>
                  <TH colIdx={0} sortKey="type" filterConfig={{
                    label: "Tipe", options: typeOptions, selected: fType,
                    onApply: (vals) => setFType(vals), onClear: () => setFType([]),
                    sortDir: sortCol === "type" ? sortDir : null,
                    onSort: (dir) => { setSortCol("type"); setSortDir(dir); },
                  }}>Tipe</TH>

                  <TH colIdx={1} sortKey="partner" filterConfig={{
                    label: "Partner", options: partnerOptions, selected: fPartner,
                  ...
                  <TH colIdx={2} sortKey="branch" filterConfig={{
                    label: "Kantor Cabang", options: branchOptions, selected: fBranch,
                    onApply: (vals) => setFBranch(vals), onClear: () => setFBranch([]),
                    sortDir: sortCol === "branch" ? sortDir : null,
                    onSort: (dir) => { setSortCol("branch"); setSortDir(dir); },
                  }}>Kantor Cabang</TH>

                  <TH colIdx={3} sortKey="region" filterConfig={{
                    label: "Region", options: regionOptions, selected: fRegion,
                    onApply: (vals) => setFRegion(vals), onClear: () => setFRegion([]),
                    sortDir: sortCol === "region" ? sortDir : null,
                    onSort: (dir) => { setSortCol("region"); setSortDir(dir); },
                  }}>Region</TH>

                  <TH colIdx={4} sortKey="done" filterConfig={{
                    label: "Jumlah Selesai", options: doneCountOptions, selected: fDoneCount,
                    onApply: (vals) => setFDoneCount(vals), onClear: () => setFDoneCount([]),
                  ...
                  {visibleMonths.map((month, i) => {
                    ...
                    return (
                      <th key={month} style={{
                        ...
                        width: colWidths[5 + i], minWidth: colWidths[5 + i],
                    ...
                    >
                      ...
                      <td style={{ padding: "9px 14px", textAlign: "center", borderRight: `1px solid ${t.lineH}` }}>
                        <span style={{
                          ...
                          {finCount}
                          <span style={{ fontSize: 11, fontWeight: 500, color: t.lo }}>/{visibleMonthCount}</span>
                        </span>
                      </td>
                      {/* Month cells */}
