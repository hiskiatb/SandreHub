 ...
 * - Menerima prop `disabledMonths` (Set<string>) dan `onMonthChange`.
 * - Jika `activeContext.month` ada di `disabledMonths`, form menampilkan
 *   overlay "Bulan Dinonaktifkan" — semua aksi edit/save diblokir.
 * - Semua logika lain (autofill, draft, finalize, dll.) tidak berubah.
 */
...
const LocalInput = React.memo(({ numericValue, onChange, style, placeholder, className, readOnly }) => {
  ...
  return (
    <input type="text" inputMode={readOnly?'text':'numeric'} placeholder={readOnly?(numericValue===0?'—':undefined):(placeholder??'0')}
      value={display} onChange={onChangeFn} onFocus={onFocus} onBlur={onBlur} readOnly={readOnly} className={className}
...
const G = ({ d, t }) => (
  <style>{`
    ...
    .fp-btn-ghost { display:inline-flex;align-items:center;gap:6px;padding:9px 13px;border-radius:9px;border:1px solid ${t.line};background:${t.sub};cursor:pointer;font-size:12px;font-weight:700;letter-spacing:0.05em;text-transform:uppercase;color:${t.mid};transition:all 0.13s;font-family:inherit; }
    .fp-btn-ghost:hover { border-color:#ED1C24;color:#ED1C24; }
    .autofill-btn { display:inline-flex;align-items:center;gap:5px;padding:5px 11px;border-radius:7px;border:1px solid ${t.violetBd};background:${t.violetBg};color:${t.violet};cursor:pointer;font-size:10px;font-weight:700;letter-spacing:0.07em;text-transform:uppercase;font-family:inherit;transition:all 0.13s; }
    .autofill-btn:hover { background:${d?'rgba(167,139,250,0.20)':'rgba(124,58,237,0.13)'};transform:translateY(-1px); }
    .autofill-btn:disabled { opacity:0.5;cursor:not-allowed;transform:none; }
    .reset-btn { display:inline-flex;align-items:center;gap:5px;padding:5px 11px;border-radius:7px;border:1px solid ${t.amberBd};background:${t.amberBg};color:${t.amber};cursor:pointer;font-size:10px;font-weight:700;letter-spacing:0.07em;text-transform:uppercase;font-family:inherit;transition:all 0.13s; }
    .reset-btn:hover { background:${d?'rgba(255,203,5,0.20)':'rgba(255,203,5,0.16)'}; }
    .prefill-badge { display:inline-flex;align-items:center;gap:4px;padding:2px 8px;border-radius:99px;background:${t.violetBg};border:1px solid ${t.violetBd};color:${t.violet};font-size:9px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase; }
  `}</style>
);
    ...
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:320, gap:16, padding:'40px 24px', textAlign:'center' }}>
      ...
      <div>
        ...
        <div style={{ fontSize:14, color:t.mid, lineHeight:1.65, maxWidth:340 }}>
          Bulan <strong style={{ color:t.hi }}>{month}</strong> untuk branch ini telah dinonaktifkan dari PNL Control Center. Laporan bulan ini tidak dapat diisi atau dilihat.
        </div>
      </div>
...
function PrefillBanner({ prevMonth, prevYear, onReset, t }) {
  return (
    <motion.div initial={{ opacity:0, y:-6, scale:0.98 }} animate={{ opacity:1, y:0, scale:1 }} exit={{ opacity:0, y:-4, scale:0.98 }} transition={{ duration:0.18 }}
  ...
  return (
    <div style={{ position:'fixed', bottom:0, left:0, right:0, borderTop:`1px solid ${t.line}`, background:d?'rgba(13,13,14,0.94)':'rgba(255,255,255,0.94)', backdropFilter:'blur(24px)', WebkitBackdropFilter:'blur(24px)', zIndex:60, padding:'11px 20px' }}>
      <div style={{ width:'100%', margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'flex-end', gap:10 }}>
        {!readOnly&&(<button onClick={onDraft} disabled={isSaving} className="fp-btn-ghost">{isSaving?<><Loader2 size={13} style={{ animation:'fpspin 1s linear infinite' }}/>Simpan...</>:<><Save size={13}/>Draft</>}</button>)}
        {step>1&&<button onClick={()=>setStep(s=>s-1)} className="fp-btn-ghost"><ArrowLeft size={13}/>Kembali</button>}
        {step<totalSteps?<button onClick={()=>setStep(s=>s+1)} className="fp-btn-primary">Lanjut <ArrowRight size={13}/></button>:(!readOnly&&<button onClick={onSubmit} disabled={isSaving} className="fp-btn-primary"><Send size={13}/>Kirim</button>)}
      </div>
    </div>
  );
}
...
const FormPengeluaran = ({
  ...
  }, [activeContext, effectiveReadOnly, setIsFormDirty]);
  ...
  const handleSaveDraft=async()=>{
    if (effectiveReadOnly||!validate()) return;
    setIsSaving(true);
    try {
  ...
  const handleSave=async()=>{
    ...
    try {
      ...
      if (!user) throw new Error('Sesi berakhir.');
      const existingNotes=reportStatus.validationNotes??'';
      let parts=existingNotes?existingNotes.split(',').filter(p=>p&&!p.startsWith('pengeluaran:')):[]; parts.push('pengeluaran:final');
  ...
  const updateVal=useCallback((section,id,field,val)=>{
    ...
    setData(prev=>{
      ...
      return { ...prev, [section]:prev[section].map(i=>{
        if (i.id!==id) return i;
  ...
  return (
    <div style={{ width:'100%', margin:'0 auto', paddingBottom:monthDisabled?0:80, fontFamily:FONT_STACK, WebkitFontSmoothing:'antialiased', color:t.hi }}>
      <G d={d} t={t}/>
      {/* Header */}
      ...
      {monthDisabled ? (
        ...
        <>
            ...
            {step===1&&(
              <motion.div key="s1" initial={{ opacity:0,y:7 }} animate={{ opacity:1,y:0 }} exit={{ opacity:0 }} transition={{ duration:0.17 }} style={{ display:'flex', flexDirection:'column', gap:14 }}>
                <SecHero icon={Building2} step={1} title="OPEX Branch" t={t}>
                  {!effectiveReadOnly&&<AutoFillButton isLoading={prefillState.opex.loading} isPrefilled={prefillState.opex.done} onClick={()=>handleAutoFill('opex')} t={t}/>}
                </SecHero>
                <AnimatePresence>{prefillState.opex.done&&!effectiveReadOnly&&<PrefillBanner prevMonth={prevMonth} prevYear={prevYear} onReset={()=>handleReset('opex')} t={t}/>}</AnimatePresence>
                <ExpenseTable items={stats.opex.items} sectionKey="opex" onUpdate={updateVal} t={t} readOnly={effectiveReadOnly} prefillKeys={prefillState.opex.done?prefillState.opex.keys:null}/>
            ...
            {step===2&&(
              <motion.div key="s2" initial={{ opacity:0,y:7 }} animate={{ opacity:1,y:0 }} exit={{ opacity:0 }} transition={{ duration:0.17 }} style={{ display:'flex', flexDirection:'column', gap:14 }}>
                <SecHero icon={Users} step={2} title="SDM Branch" t={t}>
                  {!effectiveReadOnly&&<AutoFillButton isLoading={prefillState.sdm.loading} isPrefilled={prefillState.sdm.done} onClick={()=>handleAutoFill('sdm')} t={t}/>}
                </SecHero>
                <AnimatePresence>{prefillState.sdm.done&&!effectiveReadOnly&&<PrefillBanner prevMonth={prevMonth} prevYear={prevYear} onReset={()=>handleReset('sdm')} t={t}/>}</AnimatePresence>
                <ExpenseTable items={stats.sdm.items} sectionKey="sdm" onUpdate={updateVal} t={t} readOnly={effectiveReadOnly} prefillKeys={prefillState.sdm.done?prefillState.sdm.keys:null}/>
            ...
            {step===3&&(
              <motion.div key="s3" initial={{ opacity:0,y:7 }} animate={{ opacity:1,y:0 }} exit={{ opacity:0 }} transition={{ duration:0.17 }} style={{ display:'flex', flexDirection:'column', gap:14 }}>
                <SecHero icon={Megaphone} step={3} title="Marketing" t={t}>
                  {!effectiveReadOnly&&<AutoFillButton isLoading={prefillState.marketing.loading} isPrefilled={prefillState.marketing.done} onClick={()=>handleAutoFill('marketing')} t={t}/>}
                </SecHero>
                <AnimatePresence>{prefillState.marketing.done&&!effectiveReadOnly&&<PrefillBanner prevMonth={prevMonth} prevYear={prevYear} onReset={()=>handleReset('marketing')} t={t}/>}</AnimatePresence>
                <ExpenseTable items={stats.marketing.items} sectionKey="marketing" onUpdate={updateVal} t={t} readOnly={effectiveReadOnly} prefillKeys={prefillState.marketing.done?prefillState.marketing.keys:null}/>
                {!effectiveReadOnly&&(<Card t={t}><Body><SecLabel t={t}>Lampiran Program Lain</SecLabel><p style={{ fontSize:12, color:t.mid, marginBottom:12, lineHeight:1.6 }}>Jika ada program marketing lainnya, unggah dokumen pendukung di sini.</p><UploadZone fileState={mktFile} setFileState={setMktFile} dragState={mktDrag} setDragState={setMktDrag} inputId="mkt-up" t={t} readOnly={effectiveReadOnly}/></Body></Card>)}
            ...
            {step===4&&(
              <motion.div key="s4" initial={{ opacity:0,y:7 }} animate={{ opacity:1,y:0 }} exit={{ opacity:0 }} transition={{ duration:0.17 }} style={{ display:'flex', flexDirection:'column', gap:14 }}>
                <SecHero icon={Coins} step={4} title="Cost of Money" t={t}>
                  {!effectiveReadOnly&&<AutoFillButton isLoading={prefillState.com.loading} isPrefilled={prefillState.com.done} onClick={()=>handleAutoFill('com')} t={t}/>}
                </SecHero>
                <AnimatePresence>{prefillState.com.done&&!effectiveReadOnly&&<PrefillBanner prevMonth={prevMonth} prevYear={prevYear} onReset={()=>handleReset('com')} t={t}/>}</AnimatePresence>
                <ExpenseTable items={stats.com.items} sectionKey="com" onUpdate={updateVal} t={t} readOnly={effectiveReadOnly} prefillKeys={prefillState.com.done?prefillState.com.keys:null}/>
              ...
              <motion.div key="s5" initial={{ opacity:0,y:7 }} animate={{ opacity:1,y:0 }} exit={{ opacity:0 }} transition={{ duration:0.17 }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                  <Card t={t}><Body>
                    ...
                    <div style={{ padding:'14px 16px', borderRadius:9, background:t.greenBg, border:`1px solid ${t.greenBd}`, marginBottom:effectiveReadOnly?0:14 }}>
                      <label className="lbl" style={{ color:t.green, marginBottom:7 }}>Nominal Luar Template</label>
                      <LocalInput numericValue={data.partnerExpense} onChange={v=>updateVal('partnerExpense',null,null,v)} readOnly={effectiveReadOnly}
                    ...
                    <UploadZone fileState={file} setFileState={setFile} dragState={drag} setDragState={setDrag} inputId="fp-exp-up" t={t} readOnly={effectiveReadOnly}/>
                  </Body></Card>
                  <Card t={t}><Body>
          ...
          <FooterNav step={step} setStep={setStep} totalSteps={5} readOnly={effectiveReadOnly} isSaving={isSaving} onDraft={handleSaveDraft} onSubmit={()=>setShowSubmit(true)} t={t} d={d}/>
        </>
      )}
              ...
              <div style={{ padding:28, textAlign:'center' }}>
                ...
                <div style={{ background:t.sub, borderRadius:10, padding:'13px 16px', marginBottom:18, border:`1px solid ${t.line}`, textAlign:'left', display:'flex', flexDirection:'column', gap:8 }}>
                  {[{l:'Partner',v:ctxName},{l:'Cabang',v:ctxBranch},{l:'Periode',v:`${ctxMonth} ${ctxYear}`}].map(r=>(
                    <div key={r.l} style={{ display:'flex', justifyContent:'space-between', gap:8 }}><span style={{ fontSize:11, color:t.lo }}>{r.l}</span><span style={{ fontSize:11, fontWeight:700, color:t.hi, textAlign:'right' }}>{r.v}</span></div>
                  ))}
