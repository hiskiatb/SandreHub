...
function SpmBadge({ t }) {
  return (
    <span style={{
      ...
      background: t.spmBg, border: `1px solid ${t.spmBd}`, color: t.spm,
    }}>
...
function LastUpdatedInfo({ info, updatedAt, t }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 5, fontSize: 10, color: t.lo }}>
      ...
      <span>
        Terakhir diperbarui: <strong style={{ color: t.mid }}>{fmtDate(dateStr)}</strong>
        {info?.by && <span style={{ color: t.lo }}> · {info.by}</span>}
      </span>
    </div>
  );
}
  ...
  return (
    <div style={{ padding: '14px 16px', borderRadius: 10, background: t.spmBg, border: `1px solid ${t.spmBd}` }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>
        ...
        {canReport && (
          <button className="btn-report" onClick={onReport} title="Laporkan nilai yang tidak sesuai">
            <Flag size={9} />Laporkan
          </button>
        )}
      </div>
      {children}
    ...
    return (
      <div key={isE1 ? 'e1' : 'e2'} style={{ ...(isE1 ? {} : { borderTop: `0.5px dashed ${t.magentaBd}`, background: t.magentaBg }) }}>
        ...
        <div style={{ ...grid, padding: '10px 12px' }}>
          ...
          <div><MobLabel>HPP / unit</MobLabel><input readOnly tabIndex={-1} value={new Intl.NumberFormat('id-ID').format(item.hPokok)} style={{ width: '100%', height: 32, padding: '0 8px', border: `0.5px solid ${t.roBd}`, borderRadius: 8, background: t.roBg, color: t.lo, fontSize: 12, fontWeight: 500, fontFamily: 'inherit', textAlign: 'right', outline: 'none' }} /></div>
    ...
    <div style={cardStyle}>
      ...
      {!readOnly && (
        <div style={{ padding: '6px 12px 8px', borderTop: `0.5px solid ${t.lineH}`, display: 'flex', justifyContent: 'flex-end' }}>
          {!item.hasEntry2 ? <button className="vc-add-btn" onClick={() => onAddEntry(section, item.id)}><Plus size={11} /> Entry 2</button> : null}
        </div>
      )}
    </div>
  );
...
const FormPendapatan = ({
      ...
      try {
        ...
        if (db) {
          ...
          if (Array.isArray(rd.sp_custom)) {
            s.spCustom = rd.sp_custom.filter(c => c.name || c.qty > 0 || c.hRetail > 0 || c.hPokok > 0).map(c => ({ id: c.id ?? `csp_${Date.now()}_${Math.random()}`, name: c.name ?? '', hPokok: c.hPokok ?? 0, qty: c.qty ?? 0, hRetail: c.hRetail ?? 0, qty2: c.qty2 ?? 0, hRetail2: c.hRetail2 ?? 0, hasEntry2: c.hasEntry2 ?? false }));
          }
          ...
          if (Array.isArray(rd.vc_custom)) {
            s.vcCustom = rd.vc_custom.filter(c => c.name || c.qty > 0 || c.hRetail > 0 || c.hPokok > 0).map(c => ({ id: c.id ?? `cvc_${Date.now()}_${Math.random()}`, name: c.name ?? '', hPokok: c.hPokok ?? 0, qty: c.qty ?? 0, hRetail: c.hRetail ?? 0 }));
          }
        ...
        try {
          ...
          const { data: iLogs } = await supabase
            ...
            .select('created_at, user_email, entries')
  ...
  const stats = useMemo(() => {
    ...
    const customItems = data.spCustom.map(c => { const m1 = (c.qty || 0) * ((c.hRetail || 0) - (c.hPokok || 0)); const m2 = (c.qty2 || 0) * ((c.hRetail2 || 0) - (c.hPokok || 0)); const totalModal = (c.qty || 0) * (c.hPokok || 0) + (c.qty2 || 0) * (c.hPokok || 0); const totalJual = (c.qty || 0) * (c.hRetail || 0) + (c.qty2 || 0) * (c.hRetail2 || 0); return { ...c, margin1: m1, margin2: m2, totalModal, totalJual, totalMargin: m1 + m2 }; });
    ...
    const vcCustomItems = data.vcCustom.map(c => { const m = (c.qty || 0) * ((c.hRetail || 0) - (c.hPokok || 0)); const totalModal = (c.qty || 0) * (c.hPokok || 0); const totalJual = (c.qty || 0) * (c.hRetail || 0); return { ...c, totalModal, totalJual, totalMargin: m }; });
  ...
  const mkPayload = (fin, userId, notes) => {
    ...
    const revenueData = { sp_custom: data.spCustom.filter(c => c.name || c.qty > 0 || c.hRetail > 0).map(c => ({ id: c.id, name: c.name, hPokok: c.hPokok, qty: c.qty, hRetail: c.hRetail, qty2: c.hasEntry2 ? c.qty2 : 0, hRetail2: c.hasEntry2 ? c.hRetail2 : 0, hasEntry2: c.hasEntry2 })), vc_custom: data.vcCustom.filter(c => c.name || c.qty > 0 || c.hRetail > 0).map(c => ({ id: c.id, name: c.name, hPokok: c.hPokok, qty: c.qty, hRetail: c.hRetail })) };
  ...
  const handleReport = async () => {
    ...
    try {
      ...
      await pushNotification(supabase, {
        ...
        mpc_mp3: activeContext.mpxType,
        month: activeContext.month,
        year: activeContext.year,
        dispute_field: fieldLabel,
        dispute_value: fieldValue,
        dispute_value_formatted: formatIDR(fieldValue),
        dispute_message: reportMsg.trim(),
        triggered_by: user.id,
        triggered_name: user.user_metadata?.full_name ?? '',
        triggered_email: user.email ?? '',
      });
  ...
  const addCustomSP = useCallback(() => {
    ...
    setData(prev => ({ ...prev, spCustom: [...prev.spCustom, { id: `csp_${Date.now()}`, name: '', hPokok: 0, qty: 0, hRetail: 0, qty2: 0, hRetail2: 0, hasEntry2: false }] }));
    setIsFormDirty?.(true);
  ...
  const addCustomVC = useCallback(() => {
    ...
    setData(prev => ({ ...prev, vcCustom: [...prev.vcCustom, { id: `cvc_${Date.now()}`, name: '', hPokok: 0, qty: 0, hRetail: 0 }] }));
    setIsFormDirty?.(true);
                ...
                <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} transition={{ duration: 0.15 }}
                  ...
                  <div style={{ padding: '22px 24px 8px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 10, background: t.redBg, border: `1px solid ${t.redBd}`, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Flag size={18} style={{ color: t.red }} />
                      </div>
                      <div>
                    ...
                    <div style={{ marginBottom: 16 }}>
                      ...
                      <textarea
                        ...
                        placeholder={`Jelaskan mengapa nilai ${reportFieldLabel} sebesar ${formatIDR(reportFieldValue)} ini tidak sesuai...`}
                        rows={4}
          ...
          <AnimatePresence mode="wait">
              ...
              <motion.div key="s1" initial={{ opacity: 0, y: 7 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.17 }} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <Card t={t}>
                  <Body>
                    ...
                    {!effectiveReadOnly && <div style={{ fontSize: 11, color: t.lo, marginBottom: 14, lineHeight: 1.55 }}>Produk bawaan sistem. Tap <strong style={{ color: t.blue }}>+2</strong> untuk harga retail kedua.</div>}
                    <div className="vc-table">
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, tableLayout: 'fixed' }}>
                        <TableColHead />
                        <tbody>
                          {stats.sp.items.map(item => (<React.Fragment key={item.id}><ProductRowDesktop item={item} entryIdx={1} section="sp" onUpdate={updateVal} onAddEntry={addEntry} onRemoveEntry={removeEntry} t={t} readOnly={effectiveReadOnly} />{item.hasEntry2 && <ProductRowDesktop item={item} entryIdx={2} section="sp" onUpdate={updateVal} onAddEntry={addEntry} onRemoveEntry={removeEntry} t={t} readOnly={effectiveReadOnly} />}</React.Fragment>))}
                  ...
                  <Body>
                    ...
                    <div style={{ marginTop: 14, padding: '12px 14px', borderRadius: 9, background: t.infoBg, border: `1px solid ${t.infoBd}`, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                      ...
                      <div style={{ fontSize: 11, color: t.mid, lineHeight: 1.6 }}>
                        ...
                        Nilai yang masuk ke Sales Fee dihitung dari <strong style={{ color: t.hi }}>Penjualan Saldo ke Outlet × 1.5%</strong>. Lihat estimasi potensi di halaman <strong style={{ color: t.hi }}>Sales Fee</strong>.
                      </div>
                    </div>
                  </Body>
                </Card>
            ...
            {step === 2 && (
              <motion.div key="s2" initial={{ opacity: 0, y: 7 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.17 }} style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: t.mid, marginTop: 2, lineHeight: 1.5 }}>
                        ...
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginLeft: 8, padding: '1px 7px', borderRadius: 99, background: t.infoBg, border: `1px solid ${t.infoBd}`, fontSize: 9, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: t.info }}>Info saja · tidak masuk total</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: t.mid, marginTop: 2, lineHeight: 1.5 }}>
                        ...
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginLeft: 8, padding: '1px 7px', borderRadius: 99, background: t.greenBg, border: `1px solid ${t.greenBd}`, fontSize: 9, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: t.green }}>Masuk total</span>
                      </div>
                    </div>
                  </div>
                ...
                {/* C. SLA Monthly Fee — SPM ONLY */}
                <Card t={t}><Body>
                  <SpmField
                    label="C. SLA Monthly Fee"
                    info={spmFieldsInfo.slaFee}
                    updatedAt={!spmFieldsInfo.slaFee && data.salesFee.slaFee > 0 ? reportStatus.updatedAt : null}
                    canReport={canReportDispute}
                    onReport={() => openReport('slaFee')}
                    t={t}
                  >
                    <LocalInput
                      numericValue={data.salesFee.slaFee}
                      onChange={v => updateVal('salesFee', null, 'slaFee', v)}
                ...
                {/* D. Special Program */}
                <Card t={t}><Body>
                  <SecLabel t={t}>D. Special Program</SecLabel>
                  <LocalInput numericValue={data.salesFee.specialProgram} onChange={v => updateVal('salesFee', null, 'specialProgram', v)} className={effectiveReadOnly ? 'fpi fpi-ro' : 'fpi'} readOnly={effectiveReadOnly} />
                </Body></Card>
              ...
              <motion.div key="s3" initial={{ opacity: 0, y: 7 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.17 }}>
                ...
                <Card t={t} style={{ maxWidth: 540, margin: '0 auto' }}><Body>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                    ...
                    {/* A. Hadiah Champions Club — SPM ONLY */}
                    <SpmField
                      label="A. Hadiah Champions Club"
                      info={spmFieldsInfo.champions}
                      updatedAt={!spmFieldsInfo.champions && data.rewards.champions > 0 ? reportStatus.updatedAt : null}
                      canReport={canReportDispute}
                      onReport={() => openReport('champions')}
                      t={t}
                    >
                      <LocalInput
                        numericValue={data.rewards.champions}
                        onChange={v => updateVal('rewards', null, 'champions', v)}
                        className={spmFieldReadOnly ? 'fpi-spm fpi-spm-ro fpi-spm-ro-lg' : 'fpi-spm fpi-spm-ro-lg'}
                        readOnly={spmFieldReadOnly}
                      />
                      {!isSPMUser && !effectiveReadOnly && (
                        <div style={{ marginTop: 6, fontSize: 10, color: t.lo, lineHeight: 1.5 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 14 }}>
                        ...
                        <div key={item.field} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '11px 14px', borderRadius: 9, background: t.spmBg, border: `1px solid ${t.spmBd}` }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 4 }}>
                              <span style={{ fontSize: 12, fontWeight: 600, color: t.hi }}>{item.label}</span>
                              <SpmBadge t={t} />
                            </div>
                          ...
                          <div style={{ textAlign: 'right', flexShrink: 0 }}>
                            ...
                            {canReportDispute && (
                              <button className="btn-report" style={{ marginTop: 6, fontSize: 9 }} onClick={() => openReport(item.field)}>
                                <Flag size={8} />Laporkan
                              </button>
                            )}
                          </div>
                        </div>
                    ...
                    <Card t={t}><Body>
                      ...
                      {stats.spCustom.map((c, i) => (
                        <div key={c.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '10px 0', borderBottom: `1px solid ${t.lineH}` }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span className="custom-tag" style={{ flexShrink: 0 }}><Sparkles size={7} />{i + 1}</span><div><div style={{ fontSize: 12, fontWeight: 700, color: t.hi }}>{c.name || `Produk Custom ${i + 1}`}</div><div style={{ fontSize: 10, color: t.lo }}>Pokok: {formatIDR(c.hPokok)} · {c.qty + (c.hasEntry2 ? c.qty2 : 0)} unit</div></div></div>
                          <div style={{ textAlign: 'right', flexShrink: 0 }}><div style={{ fontSize: 13, fontWeight: 700, color: c.totalMargin < 0 ? t.red : t.green, fontVariantNumeric: 'tabular-nums' }}>{formatIDR(c.totalMargin)}</div><div style={{ fontSize: 10, color: t.lo }}>dari {formatIDR(c.totalJual)}</div></div>
                        </div>
                      ))}
                      <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 12, marginTop: 4, borderTop: `1px solid ${t.violetBd}` }}><span style={{ fontSize: 11, fontWeight: 700, color: t.violet }}>Total Custom SP</span><span style={{ fontSize: 14, fontWeight: 700, color: t.violet, fontVariantNumeric: 'tabular-nums' }}>{formatIDR(stats.spCustomTotal.totalMargin)}</span></div>
                    </Body></Card>
                  )}
                    ...
                    <Card t={t}><Body>
                      ...
                      {stats.vcCustom.map((c2, i) => (
                        <div key={c2.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '10px 0', borderBottom: `1px solid ${t.lineH}` }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span className="custom-tag" style={{ flexShrink: 0 }}><Sparkles size={7} />{i + 1}</span><div><div style={{ fontSize: 12, fontWeight: 700, color: t.hi }}>{c2.name || `Voucher Custom ${i + 1}`}</div><div style={{ fontSize: 10, color: t.lo }}>Pokok: {formatIDR(c2.hPokok)} · {c2.qty} unit</div></div></div>
                          <div style={{ textAlign: 'right', flexShrink: 0 }}><div style={{ fontSize: 13, fontWeight: 700, color: c2.totalMargin < 0 ? t.red : t.green, fontVariantNumeric: 'tabular-nums' }}>{formatIDR(c2.totalMargin)}</div><div style={{ fontSize: 10, color: t.lo }}>dari {formatIDR(c2.totalJual)}</div></div>
                        </div>
                      ))}
                      <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 12, marginTop: 4, borderTop: `1px solid ${t.violetBd}` }}><span style={{ fontSize: 11, fontWeight: 700, color: t.violet }}>Total Voucher Custom</span><span style={{ fontSize: 14, fontWeight: 700, color: t.violet, fontVariantNumeric: 'tabular-nums' }}>{formatIDR(stats.vcCustomTotal.totalMargin)}</span></div>
                    </Body></Card>
                  )}
                  ...
                  <Card t={t}><Body>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, paddingTop: 12, borderTop: `1.5px solid ${t.greenBd}` }}><span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', color: t.green }}>Total Margin Produk</span><span style={{ fontSize: 18, fontWeight: 800, color: t.green, fontVariantNumeric: 'tabular-nums' }}>{formatIDR(stats.gtMg)}</span></div>
                    </div>
                  </Body></Card>
