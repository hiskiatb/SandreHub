...
export const ALLOWED = /\.(zip|kml|kmz|geojson|json)$/i;
...
// Kunci periode "yyyyMM" (sortable) dari nama berkas — dipakai UI pemilih
// periode di ConnectSourceSection (SumatraMap.jsx) utk urutkan terbaru dulu.
// null kalau nama berkas tidak mengandung pola tanggal yang bisa dikenali.
export function periodKeyFromName(name) {
  const m = String(name || "").match(/(20\d{2})[-_ ]?(0[1-9]|1[0-2])/);
