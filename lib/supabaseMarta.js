...
function makeStub() {
  ...
  return {
    auth: {
      ...
      signInWithOtp:      () => Promise.resolve({ data: { user: null, session: null }, error: devError }),
      signInWithOAuth:    () => Promise.resolve({ data: { provider: null, url: null }, error: devError }),
      verifyOtp:          () => Promise.resolve({ data: { user: null, session: null }, error: devError }),
      signUp:     () => Promise.resolve({ data: { user: null, session: null }, error: devError }),
      signOut:    () => Promise.resolve({ error: null }),
      onAuthStateChange: () => ({ data: { subscription: { unsubscribe() {} } } }),
    },
    functions: { invoke: () => Promise.resolve({ data: null, error: devError }) },
    from:    () => makeBuilder(),
    rpc:     () => makeBuilder(),
    channel: () => {
