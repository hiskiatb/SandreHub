...
import { periodFromName } from "./geoImport";

const MAX_BYTES = 60 * 1024 * 1024; // 60 MB
export const ALLOWED = /\.(xlsb|xlsx|xls|csv)$/i;
...
export async function parseSiteFile(file, onProgress) {
  ...
  onProgress?.(100);
  return { name: name.replace(/\.[^.]+$/, ""), period: periodFromName(name), sites, count: sites.length, total: rows.length };
}
