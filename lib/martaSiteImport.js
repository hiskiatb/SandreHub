...
// TIDAK membawa koordinat (lat/lng) - itu reference/master data, wajib lokal
// (lihat "Batas Wilayah (Peta)" / "Validasi Lokasi"), bukan lewat jalur ini.
...
// Koordinat (Lat/Long) SENGAJA TIDAK ada di sini - reference/master coordinate
// data wajib lokal (spec §0.2/§9.2), dikelola lewat menu "Batas Wilayah (Peta)"
// (parsing 100% browser, tidak pernah diunggah) atau "Validasi Lokasi", bukan
// lewat import List Site ini.
export const TARGET_FIELDS = [
  ...
  { key: "kecamatan", label: "Kecamatan Unik",                guesses: ["kecamatan unik"] },
  { key: "kecamatan_name", label: "Kecamatan",                 guesses: ["kecamatan"] },
  { key: "kabupaten", label: "Kabupaten",                      guesses: ["kabupaten unik", "kabupaten", "kab/kota", "kabupaten/kota"] },
  // Kecamatan Fokus - flag YES/NO per site (bukan tabel terpisah, satu
  // sumber data dgn List Site ini - keputusan user 2026-09-16). Opsional:
  // kalau kolomnya tidak ada di file yg diupload, semua site tetap "NO"
  // (lihat isY() di buildRows - default aman, bukan required field).
  { key: "kecamatan_fokus", label: "Kecamatan Fokus",           guesses: ["kecamatan fokus", "fokus kecamatan", "kec fokus"] },
  // Site LRS - flag YES/NO per site, pola SAMA PERSIS dgn Kecamatan Fokus
  // di atas (satu sumber data dgn List Site, opsional, default "NO" kalau
  // kolomnya tidak ada di file yg diupload).
  { key: "site_lrs", label: "Site LRS",                         guesses: ["site lrs", "lrs site", "lrs"] },
];
...
export function buildRows(rows, mapping) {
  ...
  for (const r of rows) {
    ...
    const base = {
      ...
      kecamatan_name: str(g(r, "kecamatan_name")),
      kabupaten: str(g(r, "kabupaten")),
      coverage: covCol ? str(r[covCol]) : null,
      // Kolom opsional - kalau tidak dipetakan (mapping[key] kosong),
      // g() balikin null, isY(null) -> false -> "NO" (aman, tidak memaksa
      // semua site jadi fokus cuma krn kolomnya belum ada di file lama).
      kecamatan_fokus: isY(g(r, "kecamatan_fokus")) ? "YES" : "NO",
      // Site LRS BUKAN flag YES/NO spt Kecamatan Fokus - kolomnya berisi
      // nilai teks kategori (mis. "LRS"), jadi disimpan APA ADANYA
      // (di-uppercase & di-trim saja) - "site unik yang bernilai LRS"
      // artinya cocokkan literal ke teks itu, bukan cek boolean.
      site_lrs: (str(g(r, "site_lrs")) || "").toUpperCase() || "NO",
    };
