...
export async function getSheetMatrix(workbook, sheetName) {
  ...
  const rawDateMatrix = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: null, blankrows: false, raw: true, cellDates: true });
  Object.defineProperty(matrix, "__rawDates", { value: rawDateMatrix, enumerable: false });
  return matrix;
}
...
export function buildPlanRows(rows, mapping) {
  ...
  const gDate = (r, key) => toIsoDate(rawDateFor(r, mapping[key]) || g(r, key));
  const out = [];
  for (const r of rows) {
