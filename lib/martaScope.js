...
// ── Multi-assignment: 1 email bisa punya >1 assignment (branch/brand
// berbeda), disimpan di mh_profile_slots. mh_profiles selalu "mirror" salah
// satu slot yg lagi aktif (kolom current_slot_id) - getMartaScope() di atas
// TIDAK berubah sama sekali, tetap baca mh_profiles apa adanya dan otomatis
// melihat slot yg lagi di-mirror. Dua helper di bawah ini KHUSUS utk UI
// switcher (dropdown "ganti assignment") - daftar slot & pindah slot aktif,
// tanpa perlu re-login (lihat mh_switch_slot_for_email di DB).
export async function getMySlots(email) {
  if (!email) return [];
  const { data, error } = await supabaseMarta.rpc("mh_my_slots_for_email", { p_email: email.toLowerCase() });
  if (error || !data) return [];
  return data.map((s) => ({
    id: s.id, role: s.role, region: s.region, brand: s.brand,
    branchId: s.branch_id, branchName: s.branch_name, fullName: s.full_name,
    coverage: s.coverage, mc: s.mc, isCurrent: !!s.is_current,
  }));
}

export async function switchSlot(email, slotId) {
  const { error } = await supabaseMarta.rpc("mh_switch_slot_for_email", { p_email: (email || "").toLowerCase(), p_slot_id: slotId });
  if (error) throw new Error(error.message || "Gagal beralih assignment");
}

/** Daftar branch_id (uuid, mh_branches) yang termasuk region tertentu. */
// -- Region lookup via mh_sites.site_id (reliable) ---------------------------
// mh_activities.branch_id -> mh_branches masih dipakai utk display nama
// branch (branchName/branchIdsInRegion di bawah), tapi utk FILTER region
// (applyMartaScope) kita pakai site_id -> mh_sites krn joinnya lebih reliable
// (lihat komentar di atas file, MARTAHUB_SPEC.md paragraf 11 open item #3).
let _siteRegionMapPromise = null;

function loadSiteRegionMap() {
  if (!_siteRegionMapPromise) {
    _siteRegionMapPromise = supabaseMarta
      .from("mh_sites")
      .select("site_id, region")
      .then(({ data, error }) => {
        const map = new Map();
        if (!error && data) for (const s of data) if (s.site_id) map.set(s.site_id, s.region);
        return map;
      });
  }
  return _siteRegionMapPromise;
}

async function siteIdsInRegion(region) {
  if (!region) return [];
  const map = await loadSiteRegionMap();
  const ids = [];
  for (const [id, r] of map) if (r === region) ids.push(id);
  return ids;
}
...
export async function applyMartaScope(query, scope) {
  ...
  if (scope.region) {
    const ids = await siteIdsInRegion(scope.region);
    query = ids.length ? query.in("site_id", ids) : query.eq("site_id", "__no_match__");
  }
